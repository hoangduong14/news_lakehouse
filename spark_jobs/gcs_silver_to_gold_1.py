# -*- coding: utf-8 -*-
"""
Silver ➜ Gold ETL on GCS + Iceberg (full-safe version)
- Upsert (MERGE) cho toàn bộ dim & fact
- check_table_exists: dùng DESCRIBE TABLE để đi qua Iceberg catalog
- Map toàn bộ khóa ngoại NULL / rỗng về -1 + tạo hàng UNKNOWN trong dimension
- dim_author lấy nguồn từ BOTH authors + articles
- Chuẩn hóa InteractionType (lower + trim) + key BIGINT nhất quán
- Coalesce cho các chỉ số đếm / độ dài
"""
...
from pyspark.sql.functions import (
    ...
    xxhash64, split, size, length, regexp_replace, lower, trim, when
)
...
def int_date_key_from_ts(ts_col):
    """Tạo khóa ngày YYYYMMDD dạng INT từ cột timestamp (không xử lý NULL)."""
    return (year(ts_col) * lit(10000) + month(ts_col) * lit(100) + dayofmonth(ts_col))


def safe_date_key(ts_col):
    """
    Tạo DateKey INT, map NULL -> -1 để không làm rơi bản ghi fact.
    """
    return when(ts_col.isNull(), lit(-1)).otherwise(int_date_key_from_ts(ts_col))
...
# =======================
# Insert UNKNOWN rows for all dimensions
# =======================

def ensure_unknown_dimension_rows(spark):
    logger.info("Đảm bảo các hàng UNKNOWN trong dimension tồn tại...")
    ...
    spark.sql(
        ...
        USING (
          SELECT CAST(-1 AS INT) AS DateKey,
                 NULL AS FullDateAlternateKey,
                 'Unknown' AS DayNameOfWeek,
                 -1 AS DayNumberOfMonth,
                 -1 AS DayNumberOfYear,
                 'Unknown' AS MonthName,
                 -1 AS MonthNumberOfYear,
                 -1 AS CalendarQuarter,
                 -1 AS CalendarYear
        ) s
        ON t.DateKey = s.DateKey
        WHEN NOT MATCHED THEN
          INSERT (DateKey, FullDateAlternateKey, DayNameOfWeek, DayNumberOfMonth, DayNumberOfYear,
                  MonthName, MonthNumberOfYear, CalendarQuarter, CalendarYear)
          VALUES (s.DateKey, s.FullDateAlternateKey, s.DayNameOfWeek, s.DayNumberOfMonth, s.DayNumberOfYear,
    ...
    spark.sql(
        ...
        USING (
          SELECT CAST(-1 AS BIGINT) AS AuthorKey,
                 '_UNKNOWN_' AS AuthorID_NK,
                 'Unknown Author' AS AuthorName
        ) s
        ON t.AuthorKey = s.AuthorKey
        WHEN NOT MATCHED THEN
          INSERT (AuthorKey, AuthorID_NK, AuthorName)
    ...
    spark.sql(
        ...
        USING (
          SELECT CAST(-1 AS BIGINT) AS TopicKey,
                 '_UNKNOWN_' AS TopicID_NK,
                 'Unknown Topic' AS TopicName
        ) s
        ON t.TopicKey = s.TopicKey
        WHEN NOT MATCHED THEN
          INSERT (TopicKey, TopicID_NK, TopicName)
          VALUES (s.TopicKey, s.TopicID_NK, s.TopicName)
        """
    )

    # dim_sub_topic UNKNOWN
    spark.sql(
        """
        MERGE INTO gold.gold_db.dim_sub_topic t
        USING (
          SELECT CAST(-1 AS BIGINT) AS SubTopicKey,
                 '_UNKNOWN_' AS SubTopicID_NK,
                 'Unknown SubTopic' AS SubTopicName,
                 CAST(-1 AS BIGINT) AS ParentTopicKey,
                 'Unknown Topic' AS ParentTopicName
        ) s
        ON t.SubTopicKey = s.SubTopicKey
        WHEN NOT MATCHED THEN
          INSERT (SubTopicKey, SubTopicID_NK, SubTopicName, ParentTopicKey, ParentTopicName)
    ...
    spark.sql(
        ...
        USING (
          SELECT CAST(-1 AS BIGINT) AS KeywordKey,
                 '_UNKNOWN_' AS KeywordID_NK,
                 'Unknown Keyword' AS KeywordText
        ) s
        ON t.KeywordKey = s.KeywordKey
        WHEN NOT MATCHED THEN
          INSERT (KeywordKey, KeywordID_NK, KeywordText)
          VALUES (s.KeywordKey, s.KeywordID_NK, s.KeywordText)
        """
    )

    # dim_reference_source UNKNOWN
    spark.sql(
        """
        MERGE INTO gold.gold_db.dim_reference_source t
        USING (
          SELECT CAST(-1 AS BIGINT) AS ReferenceSourceKey,
                 '_UNKNOWN_' AS ReferenceID_NK,
                 'Unknown Reference' AS ReferenceText
        ) s
        ON t.ReferenceSourceKey = s.ReferenceSourceKey
        WHEN NOT MATCHED THEN
          INSERT (ReferenceSourceKey, ReferenceID_NK, ReferenceText)
          VALUES (s.ReferenceSourceKey, s.ReferenceID_NK, s.ReferenceText)
        """
    )

    # dim_interaction_type UNKNOWN
    spark.sql(
        """
        MERGE INTO gold.gold_db.dim_interaction_type t
        USING (
          SELECT CAST(-1 AS BIGINT) AS InteractionTypeKey,
                 '_unknown_' AS InteractionType
        ) s
        ON t.InteractionTypeKey = s.InteractionTypeKey
        WHEN NOT MATCHED THEN
          INSERT (InteractionTypeKey, InteractionType)
...
def build_and_merge_dimensions(spark):
    ...
    # ----- dim_author (from authors + articles) -----
    author_src = (
        au_tbl.select(col("AuthorID"), col("AuthorName"))
        .unionByName(
            a_tbl.select(col("AuthorID"), col("AuthorName")),
    ...
    dim_author = (
        author_src
        .where(col("AuthorID").isNotNull() & (col("AuthorID") != ""))
    ...
    dim_topic = (
        tp_tbl
        .where(col("TopicID").isNotNull() & (col("TopicID") != ""))
    ...
    dim_sub = (
        ...
        .where(col("st.SubTopicID").isNotNull() & (col("st.SubTopicID") != ""))
        .select(
    ...
    dim_kw = (
        kw_tbl
        .where(col("KeywordID").isNotNull() & (col("KeywordID") != ""))
    ...
    dim_ref = (
        rf_tbl
        .where(col("ReferenceID").isNotNull() & (col("ReferenceID") != ""))
    ...
    if check_table_exists(spark, "silver.silver_db.comment_interactions"):
        ...
        interaction_norm = lower(trim(col("InteractionType")))
        dim_inter = (
            ci_tbl
            .select(interaction_norm.alias("InteractionType"))
            .where(interaction_norm.isNotNull() & (interaction_norm != ""))
...
def build_and_merge_facts(spark):
    ...
    fact_pub = (
        ...
        .withColumn("PublicationDateKey", safe_date_key(col("ts")))
        .withColumn("ArticlePublicationTimestamp", col("ts"))

        # AuthorKey, TopicKey, SubTopicKey: map NULL -> -1
        .withColumn(
            "AuthorKey",
            when(col("a.AuthorID").isNull() | (col("a.AuthorID") == ""), lit(-1))
            .otherwise(xxhash64(col("a.AuthorID")))
        )
        .withColumn(
            "TopicKey",
            when(col("a.TopicID").isNull() | (col("a.TopicID") == ""), lit(-1))
            .otherwise(xxhash64(col("a.TopicID")))
        )
        .withColumn(
            "SubTopicKey",
            when(col("a.SubTopicID").isNull() | (col("a.SubTopicID") == ""), lit(-1))
            .otherwise(xxhash64(col("a.SubTopicID")))
        )
    ...
    fact_kw = (
        ...
        .withColumn("ts", col("a.PublicationDate").cast("timestamp"))
        .withColumn("ArticlePublicationDateKey", safe_date_key(col("ts")))
        .withColumn("ArticleID_NK", col("a.ArticleID"))
        .withColumn(
            "KeywordKey",
            when(col("ak.KeywordID").isNull() | (col("ak.KeywordID") == ""), lit(-1))
            .otherwise(xxhash64(col("ak.KeywordID")))
        )
        .withColumn(
            "AuthorKey",
            when(col("a.AuthorID").isNull() | (col("a.AuthorID") == ""), lit(-1))
    ...
    fact_ref = (
        ...
        .withColumn(
            "ReferenceSourceKey",
            when(col("ar.ReferenceID").isNull() | (col("ar.ReferenceID") == ""), lit(-1))
            .otherwise(xxhash64(col("ar.ReferenceID")))
        )
        .withColumn(
            "AuthorKey",
            when(col("a.AuthorID").isNull() | (col("a.AuthorID") == ""), lit(-1))
    ...
    if check_table_exists(spark, "silver.silver_db.comments"):
        ...
        fact_cmt = (
            ...
            .withColumn("CommentDateKey", safe_date_key(col("ts")))
            .withColumn("ArticleID_NK", col("a.ArticleID"))
            ...
            .withColumn("IsTopComment", lit(1))  # dataset này đang là top-comment
    ...
    if check_table_exists(spark, "silver.silver_db.comment_interactions") and check_table_exists(spark, "silver.silver_db.comments"):
        ...
        interaction_norm = lower(trim(col("ci.InteractionType")))

        fact_inter = (
            ci_join
            .withColumn("ts", col("a.PublicationDate").cast("timestamp"))
            .withColumn("ArticlePublicationDateKey", safe_date_key(col("ts")))
            .withColumn("InteractionDateKey", safe_date_key(col("ts")))  # nếu có ngày interaction riêng thì thay ở đây
            .withColumn("ArticleID_NK", col("a.ArticleID"))
            .withColumn("CommentID_NK", col("c.CommentID"))
            # InteractionTypeKey: map NULL / rỗng -> -1
            .withColumn(
                "InteractionTypeKey",
                when(interaction_norm.isNull() | (interaction_norm == ""), lit(-1))
                .otherwise(xxhash64(interaction_norm))
            )
            .withColumn(
                "AuthorKey",
                when(col("a.AuthorID").isNull() | (col("a.AuthorID") == ""), lit(-1))
...
if __name__ == "__main__":
    spark = None
    try:
        logger.info("Bắt đầu ETL Silver -> Gold (full-safe)")
        spark = create_spark_session()
        ensure_gold_namespace_and_tables(spark)
        build_and_merge_dimensions(spark)
        ensure_unknown_dimension_rows(spark)   # chèn UNKNOWN sau khi load dim
