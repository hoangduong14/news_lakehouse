...
from pyspark.sql.functions import (
    ...
    map_entries, from_json, coalesce, regexp_extract, udf,
    transform_values, length, lit, when, from_unixtime, input_file_name
)
...
logger = logging.getLogger("GCS_Bronze_to_Silver_Batch")
...
CHECKPOINT_GCS_PREFIX = os.getenv("CHECKPOINT_GCS_PREFIX", f"gs://{SILVER_BUCKET}/checkpoints/silver_v2/")
...
spark = (
    ...
    .appName("GCS_Bronze_to_Silver_Iceberg_Batch")
...
def autodetect_topics_from_gcs():
    """Liệt kê thư mục con trực tiếp dưới gs://{BRONZE_BUCKET}/{DATA_SOURCE}/ làm topic (chỉ để log)."""
    try:
        jvm = spark._jvm
...
def make_articles(dfb: DataFrame) -> DataFrame:
    ...
    dfb = dfb.withColumn(
        "trimmed_subtopic",
        when(length(trim(col("sub_topic"))) > 0, trim(col("sub_topic"))).otherwise(lit(None))
    )
...
def upsert_batch(spark: SparkSession, df_batch: DataFrame, epoch_id: int, topic_name: str):
    ...
    # Bắt buộc có URL; KHÔNG filter publish_ts nữa để không mất record
    base = (
        df_batch
    ...
    for name, df in dfs.items():
        # deduplicate theo khóa chính để tránh MERGE_CARDINALITY_VIOLATION
        if name == "comments":
...
# -----------------------
# Read from Bronze (batch) and run upsert_batch
# -----------------------
def process_all_topics_from_gcs_batch(schema):
    """
    Chạy ETL dạng batch:
    - Đọc toàn bộ file JSON dưới gs://BRONZE_BUCKET/DATA_SOURCE/ (recursive).
    - Dùng TOÀN BỘ dữ liệu trong JSON (topic, sub_topic, ...).
    - Gọi upsert_batch một lần để ghi vào Iceberg.
    """
    input_path = f"gs://{BRONZE_BUCKET}/{DATA_SOURCE}/"
    logger.info(f"[BATCH] Reading once from {input_path}")
    ...
    upsert_batch(spark, df, epoch_id=0, topic_name="all_topics_batch")

# -----------------------
# Main
# -----------------------
if __name__ == "__main__":
    logger.info("Start Bronze -> Silver BATCH job (GCS -> Iceberg)")
    try:
        create_silver_namespace_and_tables()

        # Dò topics (để logging/giám sát), không ảnh hưởng ETL batch
        ...
        # Chạy ETL batch một lần cho toàn bộ dữ liệu
        process_all_topics_from_gcs_batch(news_schema)
